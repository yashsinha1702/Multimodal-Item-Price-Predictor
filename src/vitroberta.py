# --- Full Script for ViT + RoBERTa Price Prediction ---
# Description:
# This script uses a dual-encoder architecture with a Vision Transformer (ViT) for images
# and a RoBERTa model for text. The encoders are frozen, and a custom regression head
# is trained on the combined features to predict product prices.
#
# Requirements:
...
CONFIG = {
    ...
    "output_jsonl_path": "training_dataset_vit_roberta.jsonl",
    ...
    "num_workers": 4,
    "checkpoint_dir": "vit_roberta_checkpoints_224",
...
def find_latest_checkpoint(checkpoint_dir):
    # This function finds the most recently modified checkpoint file.
    list_of_files = glob.glob(os.path.join(checkpoint_dir, '*.pth')) # Simplified to any .pth file
...
def train_model(config):
    print("--- Starting Training with ViT + RoBERTa ---")
    ...
    # --- THIS IS THE FULL CHECKPOINT LOADING LOGIC ---
    if config['resume_from_checkpoint']:
        ...
        if latest_checkpoint:
            ...
            # Handle case where training was interrupted mid-epoch
            # We start from the next epoch, as step-level resume is complex
            start_epoch += 1 
    ...
    for epoch in range(start_epoch, config['epochs']):
        ...
        epoch_save_path = os.path.join(config['checkpoint_dir'], f"vit_roberta_epoch_{epoch+1}.pth")
