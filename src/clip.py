# --- Full Script for CLIP Price Prediction ---
# Description:
# This script uses a dual-encoder architecture with CLIP's vision and text models.
# It freezes the pre-trained encoders and trains a regression head on top of the
# concatenated image and text features to predict product prices.
# IMAGES ARE RESIZED TO 448x448.
#
# Requirements:
...
CONFIG = {
    ...
    "output_jsonl_path": "training_dataset_clip.jsonl",
    ...
    "num_workers": 4,
    "checkpoint_dir": "clip_checkpoints", # Use a new directory for this model
...
def find_latest_checkpoint(checkpoint_dir):
    list_of_files = glob.glob(os.path.join(checkpoint_dir, 'clip_price_predictor_epoch_*.pth'))
...
def train_model(config):
    print("--- Starting Training with CLIP Dual-Encoder ---")
    ...
    # --- THIS IS THE CHANGE ---
    # Override the default image size (224) to your desired size (448)
    # new_size = 448
    # processor.image_processor.size = {"height": new_size, "width": new_size}
    # processor.image_processor.crop_size = {"height": new_size, "width": new_size}
    # print(f"Image processor resized to {new_size}x{new_size}")
    # --------------------------
    ...
    for epoch in range(start_epoch, config['epochs']):
        ...
        # --- FIX IS HERE: Move optimizer.zero_grad() outside the accumulation logic ---
        # We will now zero the gradients at the start of every accumulation cycle.
        optimizer.zero_grad() 
        # -----------------------------------------------------------------------------
        
        train_progress = tqdm(
            enumerate(train_loader, start=start_step if epoch == start_epoch else 0),
        ...
        for step, batch in train_progress:
            ...
            # --- FIX IS HERE: Scale the loss before backward() ---
            ...
            # --- FIX IS HERE: Adjust the optimizer step and zero_grad logic ---
            if (step + 1) % config['gradient_accumulation_steps'] == 0:
                # Perform the optimizer step after accumulating gradients
                optimizer.step()
                scheduler.step()
                
                # Zero the gradients to prepare for the *next* accumulation cycle
                optimizer.zero_grad()
            # -----------------------------------------------------------------
        ...
        epoch_save_path = os.path.join(config['checkpoint_dir'], f"clip_price_predictor_epoch_{epoch+1}.pth")
        print(f"Saving model head at end of epoch {epoch+1} to {epoch_save_path}")
        torch.save({
            'epoch': epoch,
            'step': len(train_loader) - 1, # Save the last step of the epoch
