# --- Full Script for EVA-02 + DeBERTa-V3-Large Price Prediction ---
# Description:
# This script uses a top-tier dual-encoder architecture with a large EVA-02 vision model
# and a large DeBERTa V3 model for text. The encoders are frozen, and a custom regression
# head is trained on the combined features.
#
# Requirements:
...
# --- 1. Configuration ---
CONFIG = {
    # --- Using user-specified directory paths ---
    ...
    # --- MODEL CHANGE ---
    "output_jsonl_path": "training_dataset_eva02_deberta.jsonl",
    "image_model_name": "timm/eva02_large_patch14_224.mim_in22k",
    "text_model_name": "microsoft/deberta-v3-large",
    "epochs": 5,
    "batch_size": 16, # EXTREMELY REDUCED BATCH SIZE FOR VRAM
    "learning_rate": 5e-5,
    "device": "cuda:3" if torch.cuda.is_available() else "cpu",
    "num_workers": 4,
    "checkpoint_dir": "eva02_deberta_checkpoints",
    "gradient_accumulation_steps": 8, # HEAVILY INCREASED GRADIENT ACCUMULATION
...
class EVA02DeBERTaForPricePrediction(nn.Module):
    def __init__(self, image_model_name, text_model_name):
        super().__init__()
        self.image_encoder = AutoModel.from_pretrained(image_model_name, torch_dtype=torch.bfloat16, trust_remote_code=True)
        self.text_encoder = AutoModel.from_pretrained(text_model_name, torch_dtype=torch.bfloat16)

        image_embedding_dim = self.image_encoder.timm_model.num_features # e.g., 1024 for EVA-02 Large
        text_embedding_dim = self.text_encoder.config.hidden_size      # e.g., 1024 for DeBERTa Large
    ...
    def forward(self, pixel_values, input_ids, token_type_ids, attention_mask):
        ...
        # Use pooler_output for EVA-02, Swin, etc.
        image_features = image_outputs.pooler_output

        text_outputs = self.text_encoder(
            input_ids=input_ids, token_type_ids=token_type_ids, attention_mask=attention_mask
        )
        # DeBERTa uses the [CLS] token from the last hidden state
        text_features = text_outputs.last_hidden_state[:, 0]

        # --- NEW FORWARD PASS LOGIC ---
        # 2. Project features to the common dimension
        image_projected = self.image_projection(image_features)
        text_projected = self.text_projection(text_features)

        # 3. Reshape and combine into a sequence
...
def train_model(config):
    print(f"--- Starting Training with EVA-02-Large ({config['image_model_name']}) & DeBERTa-V3-Large ({config['text_model_name']}) ---")
    generate_dataset_jsonl(config)
    os.makedirs(config["checkpoint_dir"], exist_ok=True)
    
    model = EVA02DeBERTaForPricePrediction(
    ...
    # --- Corrected Code ---
# Group all parameters from the new fusion head and regression head
    ...
    for epoch in range(start_epoch, config['epochs']):
        ...
        epoch_save_path = os.path.join(config['checkpoint_dir'], f"eva02_deberta_epoch_{epoch+1}.pth")
